...
export default function AdminServicesDashboard() {
    ...
    const loadServices = async () => {
        try {
            setLoading(true)
            const response = await fetch('/api/therapies?active=all')
            if (response.ok) {
                const data = await response.json()
                setServices(data.therapies || [])
            } else {
    ...
    const handleSaveService = async () => {
        try {
            ...
            if (!formData.name || !formData.description || !formData.price || !formData.duration_minutes) {
            ...
            if (isCreating) {
                ...
                if (response.ok) {
                    const data = await response.json()
                    setServices([...services, data.therapy])
                    setIsCreating(false)
                    setError('')
                } else {
                    const data = await response.json()
                    setError(data.error || t('services.errorCreating'))
                }
                ...
                if (response.ok) {
                    ...
                    setServices(services.map(s =>
                        s.id === selectedService.id ? data.therapy : s
                    ))
                    setIsEditing(false)
                    setError('')
                } else {
                    const data = await response.json()
                    setError(data.error || t('services.errorUpdating'))
                }
            }
        } catch (error) {
            console.error('Error saving service:', error)
            setError(isCreating ? t('services.errorCreating') : t('services.errorUpdating'))
        }
    }

    const handleDeleteService = async (serviceId: number) => {
        try {
            const response = await fetch(`/api/therapies?id=${serviceId}`, {
                method: 'DELETE'
            })
            ...
            if (response.ok) {
                ...
                const data = await response.json()
                setError(data.error || t('services.errorDeleting'))
            }
        } catch (error) {
