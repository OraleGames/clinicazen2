-- Corrected Migration Script for user_role type and role column
...
-- Step 2: Update existing data in the role column to use new enum values
...
-- Step 3: Add missing columns to profiles table (if they don't exist)
DO $$
BEGIN
...
UPDATE appointments SET status = 'PENDING' WHERE status NOT IN ('PENDING', 'CONFIRMED', 'CANCELLED', 'COMPLETED', 'NO_SHOW');
...
UPDATE payments SET status = 'PENDING' WHERE status NOT IN ('PENDING', 'PAID', 'REFUNDED', 'PARTIAL_REFUND');
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE appointments ADD COLUMN payment_status payment_status DEFAULT 'PENDING';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE appointments ADD COLUMN total_amount DECIMAL(10,2) NOT NULL DEFAULT 0;
    END IF;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='appointments' AND column_name='cancellation_fee'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='appointments' AND column_name='cancellation_deadline_hours'
    ) THEN
        ALTER TABLE appointments ADD COLUMN cancellation_deadline_hours INTEGER DEFAULT 5;
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE blog_posts ADD COLUMN language TEXT NOT NULL DEFAULT 'es' CHECK (language IN ('es', 'en'));
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='therapist_availability' AND column_name='date'
    ) THEN
        ALTER TABLE therapist_availability ADD COLUMN date DATE;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='therapist_availability' AND column_name='notes'
    ) THEN
        ALTER TABLE therapist_availability ADD COLUMN notes TEXT;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='therapist_availability' AND column_name='updated_at'
    ) THEN
        ALTER TABLE therapist_availability ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='therapies' AND column_name='is_active'
    ) THEN
        ALTER TABLE therapies ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='therapies' AND column_name='category'
    ) THEN
        ALTER TABLE therapies ADD COLUMN category TEXT;
    END IF;
END $$;
...
-- Insert sample therapies if table is empty
