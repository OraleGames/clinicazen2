-- Updated Database Schema for Therapy Appointment System
-- Run this SQL code in your Supabase SQL dashboard to update the existing schema

-- Update user_role enum to remove RECEPTIONIST and add CLIENT
DROP TYPE IF EXISTS user_role CASCADE;
CREATE TYPE user_role AS ENUM ('ADMIN', 'CLIENT', 'THERAPIST');

-- Update appointment_status enum for better workflow
DROP TYPE IF EXISTS appointment_status CASCADE;
CREATE TYPE appointment_status AS ENUM ('PENDING', 'CONFIRMED', 'CANCELLED', 'COMPLETED', 'NO_SHOW');

-- Update payment_status enum for more detailed tracking
DROP TYPE IF EXISTS payment_status CASCADE;
CREATE TYPE payment_status AS ENUM ('PENDING', 'PAID', 'REFUNDED', 'PARTIAL_REFUND');

-- Add new tables that we need

-- Products table (for selling products)
...
CREATE TABLE IF NOT EXISTS payment_methods (
    ...
    provider_payment_method_id TEXT NOT NULL, -- ID from payment provider
    last_four TEXT,
...
-- Transactions table (more detailed than payments)
...
-- Add columns to services table (rename to therapies for clarity)
...
-- Add columns to appointments table for better functionality
...
-- Add columns to blog_posts table for i18n support
...
-- Update therapist_availability to be more flexible
...
-- Create a new table for specific date availability (in addition to recurring)
...
-- Update existing profiles policies to remove RECEPTIONIST references
...
-- Note: You'll need to manually handle any RECEPTIONIST users and decide whether to make them ADMIN or remove them
...
-- Insert some sample therapies if the table is empty
...
('Acupuntura', 'Medicina tradicional china con inserción de agujas finas', 45, 65.00, 'Terapias Alternativas', true),
